...
def liveness_and_interference_graph(p: src.Program, cfg: DirectedGraph[Label]) -> UndirectedGraph[Node]:
    ...
    block_live_in: dict[Label, set[Node]] = {label: set() for label in p}

    for label in cfg.iter_topological():
        block: src.Block = p[label] if label in p else ilist()
        live_out: set[Node] = set()
        ...
        for i in reversed(block):
            ...
            match i:
                case src.Jump(target):
                    live_out = block_live_in[target]
                case src.Branch(_, _, _, target):
                    live_out = live_out | block_live_in[target] | r
                case _:
                    live_out = (live_out - w) | r

            for x in r | w:
                graph.add_node(x)

            match i:
                case src.Jump(_) | src.Branch(_, _, _, _):
                    pass
                case src.Move(d, s):
                    for x in live_out:
                        if x != d and x != s:
