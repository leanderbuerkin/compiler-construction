...
import ast_5_sel
import ast_6_mem
import ast_7_patched
import ast_8_riscv
from pass_0_1_parser import parse, ParseError
from pass_1_2_shrink import shrink
from pass_2_3_monadic import monadic
from pass_3_4_explicate import explicate
from pass_4_5_select import select
from pass_5_6_assign import assign_homes
from pass_6_7_patch import patch_instructions
from pass_7_8_prelude import add_prelude_and_conclusion
...
ast = monadic(ast)
if verbose:
    print(ast_3_mon.pretty(ast))

if verbose:
    print("\n===== EXPLICATE CONTROL =====\n")
ast = explicate(ast)
if verbose:
    print(ast_4_exp.pretty(ast))

if verbose:
    print("\n===== INSTRUCTION SELECTION =====\n")
ast = select(ast)
if verbose:
    print(ast_5_sel.pretty(ast))
...
if verbose:
    print(ast_6_mem.pretty(ast))

if verbose:
    print("\n===== PATCH INSTRUCTIONS =====\n")
ast = patch_instructions(ast)
if verbose:
    print(ast_7_patched.pretty(ast))

if verbose:
    print("\n===== ADD PRELUDE & CONCLUSION =====\n")
ast = add_prelude_and_conclusion(ast, reg_alloc)
if verbose:
    print(ast_8_riscv.pretty(ast))

tgt_str = ast_8_riscv.pretty(ast)
