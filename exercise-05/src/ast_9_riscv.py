...
# Instructions
type Instr = Label | Load | LoadAddress | Store | RInstr | IInstr2 | IInstr1 | Call | Jump | Branch | Return | Directive
...
type IInstr2Name = Literal["addi", "xori", "slti", "sltiu", "slli", "srli"]
...
def pretty_instr(i: Instr) -> str:
    match i:
        ...
        case Jump(label):
            return f"\tj\t{label}"
        case Branch(nm, src1, src2, target):
            return f"\t{nm}\t{pretty_arg(src1)},{pretty_arg(src2)},{target}"
        case Return():
            return "\tret"
        case DGlobal(l):
            return f"\n\t.globl\t{l}"
