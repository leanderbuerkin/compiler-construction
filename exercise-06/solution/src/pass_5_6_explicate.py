import ast_5_mon as src
import ast_6_exp as tgt
...
def explicate_decl(d: src.Decl) -> tgt.Decl:
    match d:
        case src.DFun(src.Id(name), params, ret_ty, body):
            l_fun = Label(name)
            ...
            return tgt.DFun(tgt.Label(name), params, ret_ty, l_start, l_end, out)
# END
...
def explicate_expr(e: src.Expr) -> tgt.Expr:
    match e:
        ...
        case src.EFunRef(src.Id(name), num_params):
            return tgt.EFunRef(tgt.Label(name), num_params)
        # END
