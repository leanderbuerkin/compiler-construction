import ast_6_exp as src
import ast_7_sel as tgt
...
def select_decl(d: src.Decl) -> tgt.Function:
    match d:
        case src.DFun(label, params, _, start_label, end_label, body):
            out: tgt.Blocks = {}
            for label_, block in body.items():
                out[label_] = select_block(end_label, block)
            param_moves = IList([
                tgt.Move(x, reg)
                for (x, _), reg in zip(params, FUNCTION_ARG_REGISTERS)
            ])
...
def select_stmt(end_label: Label, s: src.Stmt) -> IList[tgt.Instr]:
    match s:
        ...
        # BEGIN
        case src.SAssign(lhs, src.EFunRef(label, num_params)):
            lhs_out = select_lhs(lhs)
            return ilist(
                # num_params not yet needed in this chapter
