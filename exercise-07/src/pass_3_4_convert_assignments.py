...
def conv_ass_decl(d: src.Decl) -> tgt.Decl:
    match d:
        case src.DFun(name, params, body):
            ...
            for variable in AF:
                # The solution uses the variable as initial value,
                # the tests succeeded also without it.
                tuple_assignment = tgt.SAssign(tgt.LId(variable), tgt.ETuple(ilist(tgt.EConst(0, '64bit'))))
                new_body += ilist(tuple_assignment)
...
def conv_ass_stmt(AF: set[Id], s: src.Stmt) -> tgt.Stmt:
    match s:
        ...
        case src.SAssign(x, e):
            e_out = conv_ass_expr(AF, e)
            if x in AF:
                return tgt.SAssign(tgt.LSubscript(tgt.EVar(x), 0), e_out)
...
def conv_ass_expr(AF: set[Id], e: src.Expr) -> tgt.Expr:
    match e:
        ...
        case src.ELambda(params, body):
            new_body = conv_ass_expr(AF, body)
            return tgt.ELambda(params, new_body, IList(list(free_vars(e))))
...
def assigned_vars_stmt(s: src.Stmt) -> set[Id]:
    match s:
        case src.SAssign(lhs, _):
            return {lhs}
        case src.SIf(_, body, orelse):
            return assigned_vars_stmts(body) | assigned_vars_stmts(orelse)
